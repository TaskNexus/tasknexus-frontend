<template>
  <div class="h-full w-full flex flex-col bg-slate-50">
    ...
    <div class="flex-1 flex min-h-0 relative">
        <!-- Graph View -->
        <template v-if="currentViewType === 'graph'">
            ...
            <div v-if="activeNode" class="w-96 shrink-0 bg-white border-l border-gray-200 flex flex-col shadow-lg z-20 overflow-hidden">
                ...
                <div class="flex-1 overflow-y-auto p-4 space-y-6">
                    ...
                    <div v-if="agentTaskId">
                        <button 
                            @click="openAgentLog"
                            class="w-full flex items-center justify-center gap-2 px-3 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-md text-sm font-medium transition-colors"
                        >
                            查看日志
                        </button>
                    </div>
...
import ClientAgentLogView from '../components/ClientAgentLogView.vue'
...
// Navigation Stack for SubProcesses & sub-pages
interface NavItem {
    label: string
    workflowId: string
    subprocessNodeId: string | null
    type: 'graph' | 'log'
    agentTaskId?: number | string | null
}
const navStack = ref<NavItem[]>([])

const currentViewType = computed(() => {
    if (navStack.value.length === 0) return 'graph'
    return navStack.value[navStack.value.length - 1].type
})

const currentAgentTaskId = computed(() => {
    if (navStack.value.length === 0) return null
    return navStack.value[navStack.value.length - 1].agentTaskId || null
})
...
const getNodeStatusClass = (status: string) => {
    ...
    switch (s) {
        ...
        case 'REVOKED': case 'CANCELLED': return 'bg-gray-100 text-gray-700'
...
const navigateBack = () => {
    ...
    if (current.type === 'graph') {
        loading.value = true
        loadGraph(current.workflowId).then(() => {
            loading.value = false
        })
    }
}
...
const navigateTo = async (index: number) => {
    ...
    // Load graph if navigating to a graph view
