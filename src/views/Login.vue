<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    ...
    <div v-if="autoLogging" class="text-center">
      <svg class="animate-spin h-8 w-8 text-blue-600 mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
      </svg>
      <p class="text-gray-500 text-sm">自动登录中...</p>
    </div>
    <div v-else class="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl">
      <div>
...
onMounted(async () => {
  ...
  // 7-day auto-login: if a refresh token exists, try to silently restore the session
  ...
  if (refreshToken) {
    ...
    if (success) {
      router.push('/')
      return // Skip the rest, user is logged in
    }
